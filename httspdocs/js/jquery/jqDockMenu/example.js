...
var DBG = function(c, x){
	...
	, exampleClicks = function(ev){
			...
			//var clicked = $(this).parent().find('img').jqDock('get').Title;
			var clicked = $(this).parent().find('img').attr('id');
			//alert(clicked);
			this.blur();
			...
			}else if(clicked == 'catalogo'){
				self.location = '/comprar-productos-natura-mexico/';
			}else if(clicked == 'consultora' || clicked == 'contacto'){
				self.location = '/consultora-natura-mexico/' + clicked;
			}else{
				self.location = '/conoce-natura/' + clicked;
			}
...
jQuery(document).ready(function($){
	...
	<!-- JS jqDockMenu LEFT -->
	// set a click handler on all items (children) within #menu...
	$('#menu2').children().click(exampleClicks);
	//show the versions...
	$('#versions2').html('jQuery v' + $().jquery + '&nbsp;&nbsp;&nbsp;&nbsp;jqDock v' + ($.jqDock ? $.jqDock.version : '?'));
	// target blank window for the html file...
	$('a.descFile2').attr('target', '_blank').addClass('targetBlank');
	// target top window for the examples page...
	$('a.descList2').attr('target', '_top');
	// just for example.php, allow expand/collapse of code samples...
	$('#description2 .exco').click(function(){
			$(this).parent().next('.codex').slideToggle();
			this.blur();
			return false;
		});
	<!-- JS jqDockMenu Right -->
	// set a click handler on all items (children) within #menu...
	$('#menu3').children().click(exampleClicks);
	//show the versions...
	$('#versions3').html('jQuery v' + $().jquery + '&nbsp;&nbsp;&nbsp;&nbsp;jqDock v' + ($.jqDock ? $.jqDock.version : '?'));
	// target blank window for the html file...
	$('a.descFile3').attr('target', '_blank').addClass('targetBlank');
	// target top window for the examples page...
	$('a.descList3').attr('target', '_top');
	// just for example.php, allow expand/collapse of code samples...
	$('#description3 .exco').click(function(){
			$(this).parent().next('.codex').slideToggle();
			this.blur();
			return false;
		});
